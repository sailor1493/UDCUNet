...
class GANFeatLoss(nn.Module):
    ...
    def __init__(self, criterion="l1", loss_weight=1.0, reduction="mean"):
        ...
        if criterion == "l1":
            ...
            raise ValueError(
                f"Unsupported loss mode: {criterion}. Supported ones are: l1|l2|charbonnier"
            )
...
@LOSS_REGISTRY.register()
class ECFNet_loss(nn.Module):
    def __init__(self, ratio=0.5):
        super(ECFNet_loss, self).__init__()
        self.charbonier = CharbonnierLoss()
        self.fft_abs = FFT_abs_L1()
        self.ratio = ratio

    def forward(self, _input, _gt):
        charbonnier_loss = self.charbonier(_input, _gt)
        fft_abs = self.fft_abs(_input, _gt)
        return charbonnier_loss + self.ratio * fft_abs
