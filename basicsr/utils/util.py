...
def check_resume(opt, resume_iter):
    ...
    if opt["path"]["resume_state"]:
        ...
        if (
            opt["path"].get("pretrain_model_g") is not None
            or opt["path"].get("pretrain_model_d") is not None
        ):
            logger.warning("pretrain_model path will be ignored during resuming.")

        # set pretrained model paths
        opt["path"]["pretrain_model_g"] = osp.join(
            opt["path"]["models"], f"net_g_{resume_iter}.pth"
        )
        logger.info(f"Set pretrain_model_g to {opt['path']['pretrain_model_g']}")

        opt["path"]["pretrain_model_d"] = osp.join(
            opt["path"]["models"], f"net_d_{resume_iter}.pth"
        )
        logger.info(f"Set pretrain_model_d to {opt['path']['pretrain_model_d']}")

        opt["path"]["pretrain_model_r"] = osp.join(
            opt["path"]["models"], f"net_r_{resume_iter}.pth"
        )
        logger.info(f"Set pretrain_model_r to {opt['path']['pretrain_model_r']}")
...
class ProgressBar(object):
    ...
    def _get_max_bar_width(self):
        ...
        if max_bar_width < 10:
            print(
                f"terminal width is too small ({terminal_width}), "
    ...
    def start(self):
        if self.task_num > 0:
            sys.stdout.write(
                f"[{' ' * self.bar_width}] 0/{self.task_num}, "
                f"elapsed: 0s, ETA:\nStart...\n"
            )
        else:
